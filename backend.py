...
class User(db.Model):
  ...
  def add(apikey, model, data):
    #update ModelCount when adding
    jsonString = data
    entity = User(apikey=apikey,
                    model=model,
                    jsonString=jsonString)
    
    entity.put()
    modelCount = ModelCount.all().filter('en_type','User').filter('apikey',apikey).filter('model', model).get()
    if modelCount:
      modelCount.count += 1
      modelCount.put()
    else:
      modelCount = ModelCount(en_type='User', apikey=apikey, model=model, count=1)
      modelCount.put()
  ...
  def get_entities(apikey, model=None, offset=0, limit=50):
    #update ModelCount when adding
    theQuery = User.all().filter('apikey',apikey)
    if model:
      theQuery = theQuery.filter('model', model)
  ...
  def clear(apikey, model):
    #update model count when clearing model on api
    count = 0
    for object in User.all().filter('apikey',apikey).filter('model', model):
      count += 1
      object.delete()
  ...
  def clearapikey(apikey):
    #update model count when clearing model on api
    count = 0
    for object in User.all().filter('apikey',apikey):
      count += 1
      object.delete()
      
    modelCount = ModelCount.all().filter('en_type','User').filter('apikey',apikey).get()
    if modelCount:
      modelCount.delete()
    result = {'items_deleted': count}
    return result
